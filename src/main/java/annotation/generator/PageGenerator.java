...
public class PageGenerator {
    ...
    private void checkCorrectFields(List<? extends Element> elements, Element page) {
        ...
        List<String> basePageElementFields = this.roundEnv.getElementsAnnotatedWith(BasePageObject.class)
            .stream()
            .flatMap(p -> ElementFilter.fieldsIn((p).getEnclosedElements()).stream())
            .map(e -> e.getSimpleName().toString())
            .toList();

        elements.forEach(field -> {
            if (isNotAnnotated(field, PageElementGen.class) && !basePageElementFields.contains(
            ...
            if (!isNotAnnotated(field, PageElementGen.class) && !basePageElementFields.contains(
                field.getSimpleName().toString())
                && field.getAnnotation(PageElementGen.class).value().isEmpty()) {
                throw new RuntimeException(
