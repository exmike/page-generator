...
import static util.Utils.getAnnotationValue;
...
public class SpecsCreator {
    ...
    private FieldSpec generateFieldSpecFromField(VariableElement field) {
        ...
        roundEnvironment.getElementsAnnotatedWith(BasePageObject.class)
            .stream()
            .map(page -> ElementFilter.fieldsIn(page.getEnclosedElements()))
            .flatMap(List::stream)
    ...
    private AnnotationSpec stepAnnotationSpec(ExecutableElement method, VariableElement field, Page page,
        ...
        return AnnotationSpec.builder(Step.class)
            .addMember("value", "$S", page.getPageName() + WHITESPACE
                + processStepName(method, field, mobileElement)
            )
            .build();
    }
    ...
    private String processStepName(ExecutableElement method, VariableElement field, MobileElementModel mobileElement) {
        String typeValue = (((DeclaredType) mobileElement.getType()).asElement()).getAnnotation(MobileElement.class)
            .value();
        String methodValue = method.getAnnotation(Action.class).value();

        String fieldValue = field.getAnnotationMirrors().toString().contains(PageElementGen.class.getName()) ?
            field.getAnnotation(PageElementGen.class).value() : getAnnotationValue(roundEnvironment, field);
